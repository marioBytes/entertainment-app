...
alias Entertainment.Media.{Category, Rating, VideoCategory, VideoRating}
...
Enum.each(test_categories, fn category -> Media.create_category(category) end)
Enum.each(test_ratings, fn rating -> Media.create_rating(rating) end)
Enum.each(test_videos, fn video ->
  %{id: c_id} = Entertainment.Repo.all(Category) |> Enum.random()
  %{id: r_id} = Entertainment.Repo.all(Rating) |> Enum.random()

  Multi.new()
  |> Multi.run(:video, fn _, _ -> Media.create_video(video) end)
  |> Multi.run(:video_category, fn _, %{video: video} ->
    changeset = %VideoCategory{video_id: video.id, category_id: c_id}
    Repo.insert(changeset)
  end)
  |> Multi.run(:video_rating, fn _, %{video_category: video_category} ->
    changeset = %VideoRating{video_id: video_category.video_id, rating_id: r_id}
    Repo.insert(changeset)
  end)
